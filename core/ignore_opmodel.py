...
class SimulateTakeOff():
  ...
  # TASK AUTOMATION
  def automate_tasks(state, t_idx):
    # Compute largest training run
    state.biggest_training_run = state.compute * state.frac_compute_training
    state.biggest_training_run = state.compute * state.frac_compute_training

    # Update number of automatable tasks
    state.automatable_tasks_goods = np.sum(state.automation_training_flops_goods < state.biggest_training_run) 
    state.automatable_tasks_rnd   = np.sum(state.automation_training_flops_rnd < state.biggest_training_run) 

    # Update compute to labour ratio of automatable tasks
    state.task_compute_to_labour_ratio_goods = 1 / state.automation_runtime_flops_goods
    state.task_compute_to_labour_ratio_rnd   = 1 / state.automation_runtime_flops_rnd
  ...
  def goods_production(self, t_idx):
    ...
    self.capital_goods = self.capital * self.frac_capital_goods
    self.labour_goods  = self.labour  * self.frac_labour_goods
    self.compute_goods = self.compute * self.frac_compute_goods
    ...
    self.compute_task_input_goods = \
      SimulateTakeOff.solve_allocation(
          ...
          self.task_compute_to_labour_ratio_goods, 
          self.automatable_tasks_goods
      )

    self.task_input_goods = self.labour_task_input_goods + self.task_compute_to_labour_ratio_goods * self.compute_task_input_goods
    
    # We substract 1 to account for the initial compute task
    self.frac_tasks_automated_goods = (np.sum(self.compute_task_input_goods > 0) - 1) / self.n_labour_tasks_goods
    ...
    no_automation_task_input_goods = \
      no_automation_labour_task_input_goods + \
      self.task_compute_to_labour_ratio_goods*no_automation_compute_task_input_goods
  ...
  def update_rnd_state(self, t_idx):
    ...
    def _update_rnd(
      ...
      adjusted_input = research_input**self.rnd_parallelization_penalty
      new_cumulative_adjusted_input = cumulative_adjusted_input + adjusted_input*self.t_step
      growth_in_cumulative_inputs = new_cumulative_adjusted_input / cumulative_adjusted_input
      ceiling_penalty = \
        (np.log10(performance_ceiling) - np.log10(current_performance)) /\
        (np.log10(performance_ceiling) - np.log10(initial_performance))
      performance_growth_rate = growth_in_cumulative_inputs**(returns * ceiling_penalty)
      new_performance = min(current_performance * performance_growth_rate, performance_ceiling)
