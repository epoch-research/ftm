...
from . import *
from . import log
from ..stats.distributions import ParamsDistribution, PointDistribution
...
def sensitivity_analysis(quick_test_mode=False, method='one_at_a_time', variance_reduction_params={}):
	...
	if method == 'combined':
		var = variance_reduction_comparison(quick_test_mode=quick_test_mode, method='variance_reduction_on_margin',
											**variance_reduction_params)
...
def variance_reduction_comparison(quick_test_mode=False, save_dir=None, restore_dir=None,
								  method='variance_reduction_on_margin'):
	params_dist = ParamsDistribution(use_ajeya_dist=False, ignore_rank_correlations=True,
									 resampling_method='resample_all')

	metric_names = SimulateTakeOff.takeoff_metrics + ['rampup_start', 'agi_year']
	parameters = [name for name, marginal in params_dist.marginals.items() if
				  not isinstance(marginal, PointDistribution)]
	mean_samples = 100
	var_samples = 100
...
def get_parameter_importance_metrics(params, metric_names):
	...
	assert (not np.any(
		model.automation_training_flops_goods[1:] < model.initial_biggest_training_run * runtime_training_max_tradeoff))
...
def one_at_a_time_comparison(quick_test_mode=False):
	...
	for parameter, row in parameter_table.iterrows():
		...
		# Run simulations
		log.info(
			f"Running simulations for parameter '{parameter}' ({current_parameter_index + 1}/{parameter_count})...")
		...
		for takeoff_metric in low_model.takeoff_metrics:
			result[
				f"{takeoff_metric}"] = f"[{low_model.takeoff_metrics[takeoff_metric]:0.2f}, {med_model.takeoff_metrics[takeoff_metric]:0.2f}, {high_model.takeoff_metrics[takeoff_metric]:0.2f}]"
		...
		result[
			"rampup_start"] = f"[{format_year(low_model, low_model.rampup_start)}, {format_year(med_model, med_model.rampup_start)}, {format_year(high_model, high_model.rampup_start)}]"
		result[
			"agi_year"] = f"[{format_year(low_model, low_model.agi_year)}, {format_year(med_model, med_model.agi_year)}, {format_year(high_model, high_model.agi_year)}]"
		...
		result[
			"doubling_times"] = f"[{low_model.doubling_times[:4]}, {med_model.doubling_times[:4]}, {high_model.doubling_times[:4]}]"
...
def write_combined_sensitivity_analysis_report(
	...
	if not skip_inputs:
		...
		inputs_table = report.add_data_frame(last_results.parameter_table, show_justifications=True,
											 nan_format=inputs_nan_format)
...
def write_sensitivity_analysis_report(
	...
	results = \
		analysis_results if analysis_results else \
			sensitivity_analysis(quick_test_mode=quick_test_mode, method=method,
		...
		def process_header(row, col, index_r, index_c, cell):
			if row == 0:
				if index_c in SimulateTakeOff.takeoff_metrics + ["doubling_times", "rampup_start", "agi_year"]:
					cell.attrib[
						'data-meaning-suffix'] = f'<br><br>Values of the metric as we set each parameter to their [conservative, best guess, aggressive] values.'
	...
	def keep_cell(row, col, index_r, index_c, cell):
		...
		col_condition = (col in fixed_cols) or (
					index_c in most_important_metrics or index_c in ['importance', 'variance_reduction'])
...
def g(dist, get_metrics, parameters, mean_samples=1, var_samples=10, processes=None, metric_arguments=None,
	  save_dir=None, restore_dir=None):
	"""
	...
	with Pool(processes=processes) as pool:
		...
		for row, outer_sample in outer_samples.iterrows():
			worker = pool.apply_async(get_metric_values, (
				get_metrics, dist, np.random.default_rng(seeder.spawn(1)[0]), var_samples,
				{name: outer_sample[name] for name in parameters}, metric_arguments
			))
...
if __name__ == '__main__':
	...
	write_sensitivity_analysis_report(report_file_path=args.output_file, report_dir_path=args.output_dir,
									  quick_test_mode=args.quick_test_mode, method=args.method,
									  variance_reduction_params=variance_reduction_params)
