...
def write_excel_report(olde_sheet_url, report_file_path=None, report_dir_path=None):
	...
	olde = {
		...
		'frac_tasks_automated_goods': clip(
			(load_sheet_range('JQ') - load_sheet_range('JQ52')) / load_sheet_range('JT8')) * 100,
		'frac_tasks_automated_rnd': clip(
			(load_sheet_range('QH') - load_sheet_range('JQ52')) / load_sheet_range('JT8')) * 100,
	}
	...
	for label, var in state_variables.items():
		...
		table = {f'{year:.1f}': [olde[var][i], modern[var][i]] for i, year in
				 enumerate(clip(model.timesteps)[:len(modern[var])])}
...
def plot_compute_decomposition(
	...
	plt.plot(clip(timesteps), transform(compute_investment), label='$ on FLOP globally', color='blue',
			 linestyle=linestyle)
	plt.plot(clip(timesteps), transform(hardware_performance), label='Hardware quality', color='orange',
			 linestyle=linestyle)
	plt.plot(clip(timesteps), transform(software), label='Software', color='green', linestyle=linestyle)
	plt.plot(clip(timesteps), transform(frac_compute_training), label='Fraction global FLOP on training', color='red',
			 linestyle=linestyle)
	plt.yscale('log')
...
def plot_compute_decomposition_comparison(start_idx, end_idx, model, olde, modern, normalize=False):
	...
	plot_compute_decomposition(
		start_idx, end_idx, reference_idx,
		modern['timesteps'], modern['compute_investment'], modern['hardware_performance'], modern['software'],
		modern['frac_compute_training'],
	...
	plot_compute_decomposition(
		start_idx, end_idx, reference_idx,
		olde['timesteps'], olde['compute_investment'], olde['hardware_performance'], olde['software'],
		olde['frac_compute_training'],
		linestyle='--', ylim=ylim, normalize=normalize,
	)
