...
class Report:
    ...
    '''))
		...
		self.head.append(et.fromstring(
			'<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.9.0/font/bootstrap-icons.css"></link>'))
		self.head.append(
			et.fromstring('<link rel="stylesheet" href="https://unpkg.com/tippy.js@6/themes/light-border.css" />'))
		self.head.append(et.fromstring(
			'<script defer="true" src="https://cdn.jsdelivr.net/npm/clipboard@2.0.10/dist/clipboard.min.js"></script>'))
		self.head.append(
			et.fromstring('<script defer="true" src="https://unpkg.com/micromodal/dist/micromodal.min.js"></script>'))
		self.head.append(
			et.fromstring('<script defer="true" src="https://unpkg.com/panzoom@9.4.0/dist/panzoom.min.js"></script>'))
	...
	def add_data_frame_modal(self, df, modal_id, index=None, show_index=True, show_index_header=False, use_render=False,
							 parent=None, show_justifications=False, **to_html_args):
		if parent is None: parent = self.default_parent

		modal = et.Element('div',
						   {'class': 'modal micromodal-slide dataframe-modal', 'id': modal_id, 'aria-hidden': 'true'})
		content_container = et.Element('div',
									   {'class': 'modal-overlay', 'tabindex': '-1', 'data-micromodal-close': 'true'})
	...
	def add_data_frame(
			self, df, index=None, show_index=True, show_index_header=False, use_render=False, parent=None,
			show_justifications=False,
			show_importance_selector=False, importance_layout='horizontal', important_rows_to_keep=[],
			important_columns_to_keep=[0], keep_cell=None, label='xxx',  # TODO Document this
		...
		if self.add_csv_copy_button:
			...
			copy_button_container.append(
				et.fromstring(f'<div class="copy-button" data-clipboard-text="{df.to_csv()}">Copy CSV</div>'))
			container.append(copy_button_container)

		parent.append(container)

		if show_importance_selector:
			self.add_importance_selector(container, importance_layout, important_rows_to_keep,
										 important_columns_to_keep, keep_cell=keep_cell, label=label, parent=parent)
	...
	def add_importance_selector(self, table_container,
								layout='horizontal', important_rows_to_keep=[], important_columns_to_keep=[],
								keep_cell=None,  # TODO Document all this
...
if __name__ == '__main__':
	...
	report.add_paragraph(
		"Lfkasjflsa aafksl fklsajf lskajf lkasjf klkadsj fldkasj flkadsj fld sjlafjdaslfj adslfjdalsfj fladsjfds.")
	...
	# Some data frame
	num = np.array(
		[[0.17899619, 0.17899619, 0.17899619, 0.33093259, 0.2076353, 0.06130814, 0.33093259, 0.2076353, 0.06130814, ],
		 ...
		 [-0.10606583, -0.10606583, -0.10606583, -0.13680513, -0.13129103, -0.03684349, -0.13680513, -0.13129103,
		  -0.03684349, ],
		 ...
		 [0.27801304, 0.27801304, 0.27801304, -0.05769304, -0.06202813, 0.04722761, -0.05769304, -0.06202813,
		  0.04722761, ], ])
