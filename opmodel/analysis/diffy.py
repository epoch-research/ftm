...
class JSModel(Model):
    ...
    ''')
		self.module.eval(
			f'simulation_result = ftm.run_simulation(bridge.transform_python_to_js_params({json.dumps(self.inputs)}))')
...
class PythonModel(Model):
	...
	def get_main_dynamic_variables(self, step_index):
		...
		variables = {
			...
			"rnd input hardware": lambda: sim.rnd_input_hardware[step_index] / sim.rnd_input_hardware[
				0] * 0.003048307243707020,
			"cumulative input hardware": lambda: sim.cumulative_rnd_input_hardware[step_index] / sim.rnd_input_hardware[
				0] * 0.003048307243707020,
			"ratio rnd input hardware": lambda: sim.rnd_input_hardware[0] ** sim.rnd_parallelization_penalty /
												sim.cumulative_rnd_input_hardware[0],
...
def diffy(
	...
	if params_url_a is None:
		params_url_a = get_option('param_table_url',
								  'https://docs.google.com/spreadsheets/d/1r-WxW4JeNoi_gCMc5y2iTlJQnan_LLCF5s_V4ZDDMkI/export?format=csv&gid=0')

	if params_url_b is None:
		params_url_b = get_option('param_table_url',
								  'https://docs.google.com/spreadsheets/d/1r-WxW4JeNoi_gCMc5y2iTlJQnan_LLCF5s_V4ZDDMkI/export?format=csv&gid=0')
  ...
  '''))
	...
	for model in [model_a, model_b]:
		...
		if model.params_url:
			pre_lines.append(
				f'    parameters: <a href="{Report.escape(model.original_params_url)}">{Report.escape(model.original_params_url)}</a>')
		else:
	...
	inputs_table = compare_dicts(model_a.inputs, model_b.inputs, var_to_lineno, add_line_numbers=False,
								 ignore_missings=ignore_missings)
	...
	if model_a.exception or model_b.exception:
		...
		for model in [model_a, model_b]:
			if model.exception:
				...
				current_section.append(et.fromstring(
					f'<pre>    {Report.escape("    ".join(traceback.format_exception(model.exception)))}</pre>'))
		...
		metrics_table = compare_dicts(model_a.get_takeoff_metrics(), model_b.get_takeoff_metrics(), var_to_lineno,
									  add_line_numbers=False, ignore_missings=ignore_missings)
		...
		step_selector = et.Element('input', {'type': 'number', 'id': 'step-selector', 'step': '1', 'min': '0',
											 'max': str(step_count - 1), 'value': '0'})
		...
		if step_count > max_steps:
			step_selector_container.append(
				et.fromstring(f'<span>(available steps: first {max_steps} steps of {step_count})</span>'))
		...
		for step_index in range(step_count):
			external_table = compare_dicts(model_a.get_main_dynamic_variables(step_index),
										   model_b.get_main_dynamic_variables(step_index), var_to_lineno,
										   ignore_missings=ignore_missings)
			internal_table = compare_dicts(model_a.get_internal_dynamic_variables(step_index),
										   model_b.get_internal_dynamic_variables(step_index), var_to_lineno,
										   ignore_missings=ignore_missings)
		...
		if isinstance(a, np.ndarray) and isinstance(b, np.ndarray):
			if len(a) != len(b):
				...
				with np.errstate(divide='ignore'):  # ignore division by zero warnings
					diff = 100 * np.max(np.abs(
						np.divide(b - a, a, where=(a > EPS) | (np.abs(b - a) > EPS))))  # avoid zero by zero division
...
def add_section(parent_element, title, add_filters=True):
	...
	if add_filters:
		section.append(et.fromstring(
			'<p>Only show differences <input class="only-show-differences" type="checkbox" checked="true"></input></p>'))
