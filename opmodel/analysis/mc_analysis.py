...
from . import *
from . import log
from ..core.utils import get_param_names, get_metric_names, get_most_important_metrics
	...
	for trial in range(n_trials):
		for i in range(max_retries):
			...
			try:
				...
				assert (not np.any(mc_model.automation_training_flops_goods[
								   1:] < mc_model.initial_biggest_training_run * runtime_training_max_tradeoff))
...
def write_takeoff_probability_table(n_trials=100, max_retries=100, input_results_filename=None):
	...
	for n in ns:
		...
		for m in ms:
			p = np.sum(
				[n_year_doubling_before_m_year_doubling(gwp, t_step, n, m) for gwp in gwps]) / results.n_trials if (
						n > m) else np.nan
			row.append(p)
		table.append(row)
...
def write_mc_analysis_report(
	...
	for n in range(1, 20):
		row = []
		for m in range(1, 20):
			p = np.sum([n_year_doubling_before_m_year_doubling(gwp, results.t_step, n, m) for gwp in
						gwps]) / results.n_trials if (n > m) else np.nan
			row.append(p)
  ...
  ''') + '</script>'))
	...
	def process_header(row, col, index_r, index_c, cell):
		if row == 0:
			if index_c in SimulateTakeOff.takeoff_metrics:
				cell.attrib[
					'data-meaning-suffix'] = f'<br><br><i>Conditional on takeoff happening before {results.t_end}.</i>'

	metrics_quantiles = pd.DataFrame(results.metrics_quantiles)
	metrics_quantiles_styled = metrics_quantiles.style.format(lambda x: x if isinstance(x, str) else f'{x:.2f}').hide(
		axis='index')
	...
	for metric in ['full_automation', 'billion_agis']:
		plot_ecdf(results.scalar_metrics[metric], limit=results.t_end - results.t_start,
				  label=metric_id_to_human[metric], color=colors[color_index], normalize=True)
		color_index += 1
	...
	for metric in ['rampup_start', 'agi_year']:
		plot_ecdf(results.scalar_metrics[metric], limit=results.t_end, label=metric_id_to_human[metric],
	...
	report.add_paragraph(
		"<span style='font-weight:bold'>Rank correlations:</span> <span data-modal-trigger='rank-correlations-modal'><i>click here to view</i>.</span>")
	...
	report.add_paragraph(
		"<span style='font-weight:bold'>Input statistics:</span> <span data-modal-trigger='input-stats-modal'><i>click here to view</i>.</span>")
