...
class SimulateTakeOff():
	...
	def continue_simulation(self, t_idx):
		...
		if not self.dynamic_t_end:
			...
			if (self.compute[t_idx - 1] < ten_billion_agi_compute) or (
					self.biggest_training_run[t_idx - 1] < full_automation_flops):
				return True
	...
	def goods_production(self, t_idx):
		...
		self.frac_tasks_automated_goods[t_idx] = \
			(np.sum(self.task_compute_to_labour_ratio_goods[t_idx] * self.compute_task_input_goods[t_idx] > 10 *
					self.labour_task_input_goods[t_idx]) - 1) \
			/ self.n_labour_tasks_goods
	...
	def hardware_rnd_production(self, t_idx):
		...
		self.frac_tasks_automated_rnd[t_idx] = \
			(np.sum(self.task_compute_to_labour_ratio_rnd[t_idx] * self.compute_task_input_hardware_rnd[t_idx] > 10 *
					self.labour_task_input_hardware_rnd[t_idx]) - 1) \
			/ self.n_labour_tasks_rnd
		## We substract 1 to account for the initial compute task
	...
	def calculate_total_inputs(self, t_idx):
		...
		self.money_spent_training[t_idx] = \
			self.compute[t_idx] * self.frac_compute_training[t_idx] / (
						self.software[t_idx] * buyable_hardware_performance)
	...
	def add_steepness(full_requirements, gap, requirements, steepness):
		...
		result = 10 ** (
					np.log10(gap_low) + np.ceil((np.log10(requirements) - np.log10(gap_low)) / steepness) * steepness)
	...
	def plot_compute_decomposition(self, new_figure=True, crop_after_agi=True):
		...
		end_idx = min(self.time_to_index(self.agi_year + 5),
					  self.t_idx) if self.agi_year is not None and crop_after_agi else self.t_idx

		plt.plot(self.timesteps[start_idx:end_idx],
				 self.compute_investment[start_idx:end_idx] / self.compute_investment[reference_idx],
				 label='$ on FLOP globally', color='blue')
		plt.plot(self.timesteps[start_idx:end_idx],
				 self.hardware_performance[start_idx:end_idx] / self.hardware_performance[reference_idx],
				 label='Hardware (FLOP/$)', color='orange')
		plt.plot(self.timesteps[start_idx:end_idx], self.software[start_idx:end_idx] / self.software[reference_idx],
				 label='Software (2020-FLOP per FLOP)', color='green')
		plt.plot(self.timesteps[start_idx:end_idx],
				 self.frac_compute_training[start_idx:end_idx] / self.frac_compute_training[reference_idx],
				 label='Fraction global FLOP on training', color='red')
	...
	def plot_compute_decomposition_bioanchors_style(self, new_figure=True):
		...
		plt.plot(self.timesteps, training_investment / training_investment[reference_idx],
				 label='Training compute investment', color='purple')
		plt.plot(self.timesteps, self.hardware_performance / self.hardware_performance[reference_idx],
				 label='Hardware performance', color='orange')
	...
	def get_summary_table(self):
		...
		doubling_time_metrics = ['hardware_performance', 'software', 'compute_investment', 'frac_compute_training',
								 'gwp', 'capital', 'labour', 'tfp_rnd', "rnd_input_software",
								 "cumulative_rnd_input_software"]
