...
class ParamsDistribution():
	...
	def __init__(self, ensure_no_automatable_goods_tasks=True, ignore_rank_correlations=False, use_ajeya_dist=True,
				 resampling_method='gap_only'):
		"""
		...
		lowest_training_requirements_rnd = \
			(marginals['initial_biggest_training_run'].b * marginals['runtime_training_max_tradeoff'].b * marginals[
				'goods_vs_rnd_requirements_training'].b) \
			* marginals['flop_gap_training'].a ** (10.5 / 7)
		...
		if use_ajeya_dist:
			...
			marginals['full_automation_requirements_training'] = SkewedLogUniform( \
				...
				max(lowest_training_requirements,
		...
		if not ignore_rank_correlations:
			for left in marginals.keys():
				for right in marginals.keys():
					...
					if isinstance(marginals[right], PointDistribution) or isinstance(marginals[left],
																					 PointDistribution):
						continue
		...
		if resampling_method == 'gap_only':
			...
			if self.ensure_no_automatable_goods_tasks:
				...
				for i, row in samples.iterrows():
					max_gap = (row['full_automation_requirements_training'] / (
							row['initial_biggest_training_run'] * row['runtime_training_max_tradeoff'])) ** (
									  7 / 10.5)
			...
			output_samples = pd.DataFrame(columns=[name for name in self.marginals], index=range(count),
										  dtype=np.float64)
			...
			while output_samples_count < count:
				...
				# statsmodels.distributions.copula.copulas throws an exception when we ask less than 2 samples from it
				# (hence the max)
				samples = self.joint_dist.rvs(max(2, samples_to_read), random_state=random_state,
											  conditions=conditions)[:samples_to_read]

				for i, sample in samples.iterrows():
					# Ensure the sample makes sense before adding it
					if self.ensure_no_automatable_goods_tasks:
						max_gap_goods = \
							(sample['full_automation_requirements_training'] / (
									sample['initial_biggest_training_run'] * sample[
								'runtime_training_max_tradeoff'])) ** (7 / 10.5)
						max_gap_rnd = \
